...
TESTS = [
    ...
    # Delivery / price promise guardrails
    ...
    # LLM-as-judge cases (style coherence scored 1–5)
    ...
    try:
        if test_type == "room_brief":
            ...
            if expected == "happy_path":
                ...
                # Deterministic check: parse total from budget_calculator result
                conn = sqlite3.connect(DB_PATH)
                conn.row_factory = sqlite3.Row
                b = dict(conn.execute("SELECT * FROM room_briefs WHERE brief_id = ?",
                                      (user_input,)).fetchone())
                conn.close()
                br = budget_calculator(
                    [id for id in get_catalog_ids() if id in output],
                    int(b["budget_inr"])
                )
            ...
            if expected == "decline":
                ...
                flag_words = ["too small","cannot fit","does not fit","impossible",
                              "insufficient space","won't fit","will not fit"]
                passed = any(w in output.lower() for w in flag_words)
            ...
            # Check db.py correctly flags NULL price items
            conn = sqlite3.connect(DB_PATH)
            conn.row_factory = sqlite3.Row
            ...
            notes  = f"{user_input} has NULL price — db correctly has no price to expose" if passed \
            ...
            notes  = f"{user_input} is out of stock — catalog_search correctly excludes it by default" \
