...
from db import catalog_search, budget_calculator, layout_fit_check  # FIX: import all 3 tools at top
...
- If asked for designer brands (Togo, Noguchi, Eames): decline and offer closest catalog alternatives
...
OUT_OF_SCOPE_TRIGGERS = [
    ...
    "wiring", "electrician", "plumbing", "pipe", "drain",
    "tile the floor", "install floor", "ceiling beam", "roof", "is it safe to remove",
...
def generate_response(user_input: str, results: list) -> str:
    if not results:
        data_str = "No matching items found in the catalog."
    else:
        ...
        messages=[
            {"role": "system", "content": SYSTEM_PROMPT},
            {
                ...
                "content": (
                    ...
                    "Show item name, price in Indian Rupees, key dimensions, and a one-line reason to choose it. "
...
# FIX: design_room is now BEFORE __main__ so Python can find it when called
def design_room(brief_id: str) -> str:
    ...
    # Normalize free-text must-haves to exact DB category names
    CATEGORY_MAP = {
        "sofa": "Sofa", "3-seater sofa": "Sofa", "2-seater sofa": "Sofa",
        "sectional": "Sofa", "couch": "Sofa",
        "full living room: sofa": "Sofa",
    ...
    # TOOL 1: catalog_search for each must-have item
    for item_type in must_haves:
        category = normalize(item_type)
...
Be honest if over budget or layout is tight. Always show prices with the rupee symbol.
"""
    response = client.chat.completions.create(
        model=MODEL,
        messages=[
...
if __name__ == "__main__":
    ...
    # --- Part 2: Full agentic room design ---
    print("\n\n" + "=" * 60)
